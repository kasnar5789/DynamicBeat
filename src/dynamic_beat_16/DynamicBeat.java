...
public class DynamicBeat extends JFrame {
	...
	public DynamicBeat() {
		...
		startButton.setBounds(40, 200, 400, 100);	//시작 버튼 크기,위치 조절
		startButton.setBorderPainted(false);		//버튼의 외각선 없애줌
		startButton.setContentAreaFilled(false);	//버튼의 내용영역 채우기 안함
		startButton.setFocusPainted(false);			//버튼이 선택되었을때 생기는 테두리 사용안함
		startButton.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseEntered(MouseEvent e) {
				startButton.setIcon(startButtonEnteredImage);	// 컴포넌트에 마우스가 들어올 경우 이미지 출력
				startButton.setCursor(new Cursor(Cursor.HAND_CURSOR));	// 커서가 손모양으로 바뀜
				Music buttonEnteredMusic = new Music("buttonEnteredMusic.mp3", false);	// 소리 재생
				buttonEnteredMusic.start();
			}
			@Override
			public void mouseExited(MouseEvent e) {
	...
	public void selectTrack(int nowSelected) {
		if(selectedMusic != null)
			selectedMusic.close();			// 어떠한 곡이 실행되고 있다면 종료
		titleImage = new ImageIcon(Main.class.getResource("../images/" + trackList.get(nowSelected).getTitleImage())).getImage();	//현재 선택된 곡이 가지고있는 타이틀이미지 값을 가져옴
		selectedImage = new ImageIcon(Main.class.getResource("../images/" + trackList.get(nowSelected).getStartImage())).getImage();	// "시작이미지"
	...
	public void gameStart(int nowSelected, String difficulty) {
		...
		leftButton.setVisible(false);	//버튼 가림
		rightButton.setVisible(false);
		easyButton.setVisible(false);
		hardButton.setVisible(false);
		...
		isGameScreen = true;		// 게임화면임을 알림
		game = new Game(trackList.get(nowSelected).getTitleName(), difficulty, trackList.get(nowSelected).getGameMusic());
		game.start();
		setFocusable(true);		//키이벤트의 포커스를 우선적으로 받음
	}
